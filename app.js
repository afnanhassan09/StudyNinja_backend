...
const { checkAndResetStaleEssays } = require("./controllers/tutorController");
  ...
  socket.on("sendMessage", async ({ sender, recipient, content }) => {
    try {
      ...
      if (senderStudent) senderModel = "Student";
        ...
        throw new Error(
          `Sender with ID ${sender} not found in Student or Tutor collection.`
        );
      ...
      if (recipientStudent) recipientModel = "Student";
        ...
        throw new Error(
          `Recipient with ID ${recipient} not found in Student or Tutor collection.`
        );
...
cron.schedule("*/10 * * * *", async () => {
  try {
    ...
    const response = await axios.get(
      "https://studyninja-backend.onrender.com/"
    );
