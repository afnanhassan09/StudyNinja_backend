...
class StudentController {
    ...
    try {
      ...
      if (existingStudent) {
        ...
        existingStudent.profilePicture =
          profilePictureUrl || existingStudent.profilePicture;
  ...
  async getAllChatContacts(req, res) {
    try {
      ...
      const uniqueTutorIds = Array.from(
        new Set([...messagePartners, ...additionalPartners])
      );
      const tutorIds = uniqueTutorIds.filter(
        (id) => id.toString() !== student._id.toString()
      );

      const tutors = await Tutor.find({ _id: { $in: tutorIds } }).populate(
        "userId",
  ...
  async getAllContacts(req, res) {
    try {
      ...
      const uniqueTutorIds = Array.from(
        new Set([...tutorIds, ...additionalTutorIds])
      );
  ...
  async uploadEssay(req, res) {
    try {
      const { title, subject, academicLevel, studentRequest, comments } =
        req.body;
      const files = req.files;
      ...
      await Promise.all([
        file
          ? uploadFile(file.buffer, file.originalname, file.mimetype).then(
              (url) => (uploadedFile = url)
            )
          : null,
        markingScheme
          ? uploadFile(
              markingScheme.buffer,
              markingScheme.originalname,
              markingScheme.mimetype
            ).then((url) => (uploadedMarkingScheme = url))
          : null,
      ]);
      ...
      const isPlus18 =
        dateOfBirth &&
        new Date(dateOfBirth).getFullYear() < new Date().getFullYear() - 18;
      ...
      return res.status(200).json({
        message: "Essay uploaded successfully! Redirect to payment.",
  ...
  async confirmPayment(req, res) {
    try {
      ...
      console.log(session.metadata.essayId);
      if (session.payment_status === "paid") {
        await Essay.findOneAndUpdate(
          { _id: session.metadata.essayId },
          { paymentStatus: "Paid" }
        );

        return res.status(200).json({
          success: true,
          message: "Payment confirmed! Essay is now visible to tutors.",
        });
      }
  ...
  async getAllEssays(req, res) {
    try {
      ...
      if (essays.length === 0) {
        return res
          .status(400)
          .json({ message: "No essays found for the given student." });
      }
  ...
  async getPendingEssays(req, res) {
    try {
      ...
      if (essays.length === 0) {
        return res
          .status(200)
          .json({ message: `No prending essays found for the given student.` });
      }
  ...
  async getInProgressEssays(req, res) {
    try {
      ...
      if (essays.length === 0) {
        return res.status(404).json({
          message: `No In Progress essays found for the given student.`,
        });
      }
  ...
  async getCompletedEssays(req, res) {
    try {
      ...
      if (essays.length === 0) {
        return res.status(404).json({
          message: `No Completed essays found for the given student.`,
        });
      }
    ...
    try {
      ...
      if (!tutorId || !startTime || !endTime) {
        return res.status(401).json({
          message: "Missing required fields: tutorId, startTime, and endTime.",
        });
      }
      ...
      const dayOfWeek = startTimeObj
        .toLocaleString("en-us", { weekday: "long" })
        .toLowerCase();
      console.log("Day of Week", dayOfWeek);
      // Format the time range as "HH:mm-HH:mm"
      const timeRange = `${startTimeObj
        .toISOString()
        .slice(11, 16)}-${endTimeObj.toISOString().slice(11, 16)}`;
      ...
      if (!dayAvailability.includes(timeRange)) {
        return res.status(401).json({
          message: `Time slot ${timeRange} is not available on ${dayOfWeek}.`,
        });
      }
      // Remove the time slot
      existingSession.availability[dayOfWeek] = existingSession.availability[
        dayOfWeek
      ...
      schedule.scheduleJob(startTimeObj, async () => {
        try {
          ...
          console.log(
            `Zoom meeting created and availability restored for session ID: ${tutoringSession._id}`
          );
        } catch (error) {
          console.error(
            `Failed to create Zoom meeting for session ID: ${tutoringSession._id}`,
            error.message
          );
        }
      });
      ...
      console.error(error);
      res.status(500).json({
        message: "An error occurred while creating tutoring session.",
      });
    }
  }
  ...
  async getStudentDashboard(req, res) {
    try {
      ...
      const totalPercentage = submittedEssays.reduce(
        (sum, essay) => sum + (essay.percentage || 0),
        0
      );
      const averagePercentage =
        essaysSubmitted > 0
          ? (totalPercentage / essaysSubmitted).toFixed(1)
          : 0;
  ...
  async giveRating(req, res) {
    try {
      ...
      if (!["essay", "tutoring"].includes(type)) {
        return res.status(400).json({
          message: 'Invalid rating type. Must be either "essay" or "tutoring".',
        });
      }
      ...
      if (type === "essay" && !essayId) {
        return res
          .status(400)
          .json({ message: "Essay ID is required for essay ratings." });
      }
      if (type === "tutoring" && !tutoringId) {
        return res.status(400).json({
          message: "Tutoring session ID is required for tutoring ratings.",
        });
      }
      ...
      if (type === "essay") {
        ...
        if (!relatedObject) {
          return res
            .status(404)
            .json({ message: "Tutoring session not found." });
        }
      ...
      if (!relatedObject.studentID) {
        return res
          .status(400)
          .json({ message: "The related object is missing a student field." });
      }

      if (relatedObject.studentID.toString() !== student._id.toString()) {
        return res
          .status(403)
          .json({ message: "You are not authorized to rate this item." });
      }

      if (relatedObject.rated) {
        return res
          .status(400)
          .json({ message: "This item has already been rated." });
      }
  ...
  async getPendingRating(req, res) {
    try {
      ...
      const tutoring = await TutoringSession.find({
        student: student._id,
        rated: false,
      });
  ...
  async getEssaybyID(req, res) {
    try {
      ...
      if (essay.studentID.toString() !== student._id.toString()) {
        return res
          .status(403)
          .json({ message: "You are not authorized to view this essay." });
      }
  ...
  async getTutoringSessions(req, res) {
    try {
      ...
      console.error(error);
      res.status(500).json({
        message: "An error occurred while fetching tutoring sessions.",
      });
    }
  }
  ...
  async getUpcomingTutoringSessions(req, res) {
    try {
      ...
      if (!tutoringSessions || tutoringSessions.length === 0) {
        return res
          .status(404)
          .json({ message: "No upcoming tutoring sessions found." });
      }
